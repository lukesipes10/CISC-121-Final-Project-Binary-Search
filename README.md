...
Binary search is one of the most efficient searching algorithms. Instead of checking every suspect one by one (linear search, O(n)), binary search cuts the search space in half with every guess, solving the problem in O(log n) time. The mystery/detective theme makes this concept intuitive by making it very hard (and mostly up to luck) to solve the challenge using linear search, and that finding the value is only feasibly possible each time using the binary search function.
